...
public class IbmPCTest extends TestCase {
	...
	public void testBIOS81() throws IOException {
		...
		// public void checkpointPassed(Processor cpu) {
		// logger.info("in STGTST AX=" + Integer.toHexString(cpu.eax));
		// }
		// }));
		...
		cpList.add(new Checkpoint(0xFE5CB, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("KBD_RESET called");
			}
		}));
		cpList.add(new Checkpoint(0xFE5D8, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("SP_TEST called");
			}
		}));
		cpList.add(new Checkpoint(0xFE227, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("TEST 06: 8259 INTERRUPT CONTROLLER TEST");
			}
		}, false));
		cpList.add(new Checkpoint(0xFE277, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("TEST 07: 8253 TIMER CHECKOUT");
			}
		}, false));
		cpList.add(new Checkpoint(0xFE294, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("TEST 07 PHASE II");
			}
		}, false));
		cpList.add(new Checkpoint(0xFE2AA, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("TEST 07 TEMP INTERRUPT CALLED");
			}
		}, true));
		cpList.add(new Checkpoint(0xFE2B6, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("NMI INTERRUPT CALLED");
			}
		}, true));
		...
		cpList.add(new Checkpoint(0xFE5F8, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("Write_String_CR_LF called with si=" + Integer.toHexString(cpu.esi));
			}
		}, true));
		cpList.add(new Checkpoint(0xFE402, new CheckpointCallback() {
			public void checkpointPassed(Processor cpu) {
				logger.info("XLAT_PRINT_CODE al=" + Integer.toHexString(cpu.eax & 0x0f));
			}
		}, true));
		...
		CheckpointProcessor.setCheckpoints(cpList);
