package com.binaryorder.embeddedpc.emulator.pci.peripheral;
...
public class CGAVideoCard extends AbstractPCIDevice implements VideoCard, IOPortCapable, HardwareComponent {
	...
	private static final int[] sequencerRegisterMask = new int[] { 0x03, // ~0xfc,
			0x3d, // ~0xc2,
			0x0f, // ~0xf0,
			0x3f, // ~0xc0,
			0x0e, // ~0xf1,
	...
	private static final int[] graphicsRegisterMask = new int[] { 0x0f, // ~0xf0
			0x0f, // ~0xf0
			0x0f, // ~0xf0
			0x1f, // ~0xe0
			0x03, // ~0xfc
	...
	private static final int[] mask16 = new int[] { 0x00000000, 0x000000ff, 0x0000ff00, 0x0000ffff, 0x00ff0000,
			0x00ff00ff, 0x00ffff00, 0x00ffffff, 0xff000000, 0xff0000ff, 0xff00ff00, 0xff00ffff, 0xffff0000, 0xffff00ff,
			0xffffff00, 0xffffffff };

	private static final int[] dmask16 = new int[] { 0x00000000, 0xff000000, 0x00ff0000, 0xffff0000, 0x0000ff00,
			0xff00ff00, 0x00ffff00, 0xffffff00, 0x000000ff, 0xff0000ff, 0x00ff00ff, 0xffff00ff, 0x0000ffff, 0xff00ffff,
			0x00ffffff, 0xffffffff };

	private static final int[] dmask4 = new int[] { 0x00000000, 0xffff0000, 0x0000ffff, 0xffffffff };

	private static final int[] cursorGlyph = new int[] { 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
	...
	private VGARAMIORegion ioRegion;
	private Memory characterGenerator = null;
	...
	public CGAVideoCard() {
		...
		// ioRegion = new VGALowMemoryRegion();
	...
	public IORegion[] getIORegions() {
		return new IORegion[] { null };
	}

	public IORegion getIORegion(int index) {
		if(index == 0)
			return null;// ioRegion;
		else
			return null;
	}
	...
	public int[] ioPortsRequested() {
		return new int[] { 0x3b4, 0x3b5, 0x3ba, 0x3d4, 0x3d5, 0x3d8, 0x3da, 0x3c0, 0x3c1, 0x3c2, 0x3c3, 0x3c4, 0x3c5,
				0x3c6, 0x3c7, 0x3c8, 0x3c9, 0x3ca, 0x3cb, 0x3cc, 0x3cd, 0x3ce, 0x3cf, 0x1ce, 0x1cf, 0xff80, 0xff81 };
	}

	private final void vgaIOPortWriteByte(int address, int data) {
		// if((address >= 0x3b0 && address <= 0x3bf &&
		// ((miscellaneousOutputRegister & MOR_COLOR_EMULATION) != 0))
		// || (address >= 0x3d0 && address <= 0x3df &&
		// ((miscellaneousOutputRegister & MOR_COLOR_EMULATION) == 0)))
		// return;
		...
		case 0x3d8:
			...
			attributeRegisterIndex |= 0x20;
			graphicsRegister[GR_INDEX_MISC] |= mode == 0 ? 0 : 1;
			break;
		case 0x3b5:
		case 0x3d5:
	...
	private final int vgaIOPortReadByte(int address) {
		...
		case 0x3da:
			...
			if(updatingScreen) {
				st01 &= ~ST01_V_RETRACE; // claim we are not in vertical
				// retrace (in the process of screen
				// refresh)
				st01 &= ~ST01_DISP_ENABLE; // is set when in h/v retrace (i.e.
				// if e-beam is off, but we claim
				// always on)
			} else {
				st01 ^= (ST01_V_RETRACE | ST01_DISP_ENABLE); // if not
				// updating
				// toggle to
				// fool polling
				// in some vga
	...
	private final void vbeIOPortWriteData(int data) {
		if(vbeIndex <= VBE_DISPI_INDEX_NB) {
			...
			case VBE_DISPI_INDEX_BPP:
				if(data == 0)
					data = 8;
				if(data == 4 || data == 8 || data == 15 || data == 16 || data == 24 || data == 32) {
					vbeRegs[vbeIndex] = data;
				}
				break;
			...
			case VBE_DISPI_INDEX_ENABLE:
				if((data & VBE_DISPI_ENABLED) != 0) {
					...
					crtRegister[CR_INDEX_CRTC_MODE_CONTROL] |= 0x3; /*
					 * no CGA
					 * modes
					 */
					...
					crtRegister[CR_INDEX_OVERFLOW] = (crtRegister[CR_INDEX_OVERFLOW] & ~0x42) | ((h >>> 7) & 0x02)
							| ((h >>> 3) & 0x40);
					...
					if(vbeRegs[VBE_DISPI_INDEX_BPP] == 4) {
						shiftControl = 0;
						sequencerRegister[SR_INDEX_CLOCKING_MODE] &= ~0x8; /*
						 * no
						 * double
						 * line
						 */
						...
						shiftControl = 2;
						sequencerRegister[SR_INDEX_SEQ_MEMORY_MODE] |= 0x08; /*
						 * set
						 * chain
						 * 4
						 * mode
						 */
						sequencerRegister[SR_INDEX_MAP_MASK] |= 0x0f; /*
						 * activate
						 * all
						 * planes
						 */
					}
					graphicsRegister[GR_INDEX_GRAPHICS_MODE] = (graphicsRegister[GR_INDEX_GRAPHICS_MODE] & ~0x60)
							| (shiftControl << 5);
					crtRegister[CR_INDEX_MAX_SCANLINE] &= ~0x9f; /*
					 * no double
					 * scan
					 */
				} else {
	...
	public class VGALowMemoryRegion extends Memory {
		...
		public void setByte(int offset, byte data) {
			...
			case 3:
				offset -= 0x18000;
				// should be (unsigned) if (offset >= 0x8000) but anding above
				// "offset &= 0x1ffff;" means <=> the below
				if(offset < 0)
					return;
				break;
			}
			...
			if((sequencerRegister[SR_INDEX_SEQ_MEMORY_MODE] & 0x08) != 0) {
				...
				case 1:
					...
					offset <<= 2;
					ioRegion.setDoubleWord(offset, (ioRegion.getDoubleWord(offset) & ~writeMask)
							| (intData & writeMask));
					return;
				case 2:
	...
	public static class VGARAMIORegion extends MemoryMappedIORegion {
		...
		public void copyContentsInto(int address, byte[] buf, int off, int len) {
			System.out.println("address = " + address + ". offset = " + off + ". len = " + len + ". buffer.length = "
					+ buffer.length);
	...
	public final void updateDisplay(GraphicsDisplay device) {
		...
		// System.out.println("graphicMode = " + graphicMode);
		...
		for(int charY = 0; charY < height; charY++) {
			...
			for(int charX = 0; charX < width; charX++) {
				...
				if(fullUpdate || (charShort != this.lastChar[lastCharOffset])) {
					...
					// }
					int glyphOffset = character * 8;// fontOffset[(characterAttribute
					// >>> 3) & 1] + 32 * 4 *
					// character;
	...
	abstract class GraphicsUpdater {
		...
		void updateDisplay(GraphicsDisplay device, int width, int height, int dispWidth, boolean fullUpdate,
				int multiScan) {
			int multiRun = multiScan;
			int addr1 = 4 * startAddress;
			// int lineSize = width; // get the line size from the display
			// device??

			// if the "cursor_invalidate" function pointer is not null, then
			// call it here.
			...
			for(int y = 0; y < height; y++) {
				...
				for(int i = pageStart; i <= pageEnd; i++) {
					if(update |= ioRegion.pageIsDirty(i)) {
						...
						// if the "cursor_draw_line" function pointer is not
						// null, then call it here.
	...
	private final boolean updatePalette16(GraphicsDisplay device) {
		...
		for(int colorIndex = AR_INDEX_PALLETE_MIN; colorIndex <= AR_INDEX_PALLETE_MAX; colorIndex++) {
			...
			v *= 3;
			int col = device.rgbToPixel(c6to8(this.palette[v]), c6to8(this.palette[v + 1]), c6to8(this.palette[v + 2]));
	...
	private final boolean updateBasicParameters() {
		...
		/* line compare */
		int curLineCompare = crtRegister[CR_INDEX_LINE_COMPARE] | ((crtRegister[CR_INDEX_OVERFLOW] & 0x10) << 4)
				| ((crtRegister[CR_INDEX_MAX_SCANLINE] & 0x40) << 3);

		if((curLineOffset != this.lineOffset) || (curStartAddress != this.startAddress)
				|| (curLineCompare != this.lineCompare)) {
	...
	private final void drawGlyph8(int[] buffer, int startOffset, int scanSize, int glyphOffset, int charHeight,
			int foregroundColor, int backgroundColor) {
		int xorColor = backgroundColor ^ foregroundColor;
		scanSize -= 8;

		do {
			int fontData = characterGenerator.getByte(0xA6E + glyphOffset);
			...
			for(int i = 7; i >= 0; i--) {
				...
				// buffer[startOffset + scanSize] = pixel;
				// startOffset++;
			}
			glyphOffset += 1;
	...
	public boolean initialised() {
		return ioportRegistered && memoryRegistered && characterGenerator != null;
	}
	...
	public void updateComponent(HardwareComponent component) {
		...
		if((component instanceof PhysicalAddressSpace) && component.updated()) {
			((PhysicalAddressSpace) component).mapMemoryRegion(ioRegion, 0xb8000, 16384);
			characterGenerator = ((PhysicalAddressSpace) component).getReadMemoryBlockAt(0xFFA6E);
			for(int i = 0; i < 16; i++) {
				for(int j = 0; j < 16; j++) {
					System.out.print(Integer.toHexString(characterGenerator.getByte(i * 16 + j)) + " ");
				}
				System.out.println();
			}
	...
	public void acceptComponent(HardwareComponent component) {
		...
		if((component instanceof PhysicalAddressSpace) && component.initialised()) {
			...
			for(int i = 0; i < 16; i++) {
				for(int j = 0; j < 16; j++) {
					System.out.print(Integer.toHexString(0xff & characterGenerator.getByte(0xA6E + i * 16 + j)) + " ");
				}
				System.out.println();
			}
			memoryRegistered = true;
	...
	public String toString() {
		return "CGA Card [Mode: " + lastScreenWidth + " x " + lastScreenHeight + "]";
	}
}
