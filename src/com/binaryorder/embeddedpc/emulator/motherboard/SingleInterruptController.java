package com.binaryorder.embeddedpc.emulator.motherboard;
...
public class SingleInterruptController implements InterruptController {
	...
	private class InterruptControllerElement {
		...
		private int pollRead(int address) {
			int ret = this.getIRQ();
			if(ret < 0) {
				SingleInterruptController.this.updateIRQ();
				return 0x07;
			}

			if(0 != (address >>> 7)) {
				SingleInterruptController.this.masterPollCode();
			}
